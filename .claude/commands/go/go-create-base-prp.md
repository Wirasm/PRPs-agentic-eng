...
Generate a complete PRP for Go feature implementation with deep and thorough research. Ensure rich context is passed to the AI through the PRP to enable one pass implementation success through self-validation and iterative refinement.
...
1. **Codebase Analysis in depth**
   ...
   - Note all existing conventions to follow (Go patterns, goroutine patterns, etc.)
   - Check existing test patterns for validation approach (go test, benchmarks, etc.)
   - Use the batch tools to spawn subagents to search the codebase for similar features/patterns

2. **External Research at scale**
   - Create clear todos and spawn with instructions subagents to do deep research for similar features/patterns online and include urls to documentation and examples
   - Package documentation (include specific URLs for Go packages)
   ...
   - Implementation examples (GitHub/StackOverflow/blogs with Go focus)
...
- **Gotchas**: Package quirks, version issues, Go gotchas
